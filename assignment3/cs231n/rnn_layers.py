...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    pass
    x_hidden = x.dot(Wx)
    h_hidden = prev_h.dot(Wh)
    next_h = np.tanh(x_hidden + h_hidden + b)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    h,cache = [],[]
    for i in range(T):
        next_h, next_c, h_cache = lstm_step_forward(x[:,i,:], prev_h, prev_c, Wx, Wh, b)
        h.append(next_h)
...
def lstm_backward(dh, cache):
    ...
    N, T, H = dh.shape
    D = cache[0][4].shape[1]
    step_dprev_c,step_dprev_h = np.zeros((N, H)), np.zeros((N, H))
    ...
    for i in range(T-1,-1,-1):
        ...
        dx.append(step_dx)
        dWx, dWh, db = dWx + step_dWx, dWh + step_dWh, db + step_db
    dh0 = step_dprev_h
    dx = np.array(dx[::-1]).transpose(1, 0, 2)
    ##############################################################################
    #                               END OF YOUR CODE                             #

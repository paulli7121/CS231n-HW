...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        #(2)
        captions_in_emb, in_emb_cache = word_embedding_forward(captions_in, W_embed)
        #(3)
        if self.cell_type == 'rnn':
            h, h_cache = rnn_forward(captions_in_emb, h0, Wx, Wh, b)
        if self.cell_type == 'lstm':
            h, h_cache = lstm_forward(captions_in_emb, h0, Wx, Wh, b)
        #(4)
        ...
        if self.cell_type == 'rnn':
            dcaptions_in_emb, dh0, grads['Wx'], grads['Wh'], grads['b'] = rnn_backward(dtemp, h_cache)
        
        if self.cell_type == 'lstm':
            dcaptions_in_emb, dh0, grads['Wx'], grads['Wh'], grads['b'] = lstm_backward(dtemp, h_cache)
        
        grads['W_embed'] = word_embedding_backward(dcaptions_in_emb, in_emb_cache)
    ...
    def sample(self, features, max_length=30):
        ...
        for t in range(1, max_length):
            embed_x, _ = word_embedding_forward(x, W_embed)
            if self.cell_type == 'rnn':
